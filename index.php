    ...
    <body class="bg-carbon">
        ...
        <script type="text/javascript">
            ...
            function markdownRender() {
                ...
                // Headings
                str = $("#markdown").text();
                str = str.replace(/^#(.*)$/gmi, '<h1>$1<\/h1>');
                $("#markdown").html(str);
                console.log("Completed markdown. " + str);

                str = $("#markdown").text();
                str = str.replace(/^##(.*)$/gmi, '<h2>$1<\/h2>');
                $("#markdown").html(str);

                str = $("#markdown").text();
                str = str.replace(/^###(.*)$/gmi, '<h3>$1<\/h3>');
                $("#markdown").html(str);

                str = $("#markdown").text();
                str = str.replace(/^####(.*)$/gmi, '<h4>$1<\/h4>');
                $("#markdown").html(str);

                str = $("#markdown").text();
                str = str.replace(/^#####(.*)$/gmi, '<h5>$1<\/h5>');
                $("#markdown").html(str);

                str = $("#markdown").text();
                str = str.replace(/^######(.*)$/gmi, '<h6>$1<\/h6>');
                $("#markdown").html(str);

                // Code Blocks
                str = $("#markdown").text();
                str = str.replace(/(?:^|\n)(?:{0,3})(```+|~~~+)(?:*)([^\s`~]*)\n([\s\S]*?)\n(?:{0,3})\1/gm, '<code>$3<\/code>');
                $("#markdown").html(str);

                // HTML elements
                str = $("#markdown").text();
                str = str.replace(/^{0,2}(?-){3,}[\t]*$/gm, '<hr>');
                $("#markdown").html(str);
                ...
                // Strikethrough
                str = $("#markdown").text();
                str = str.replace(/(^|[^\\])(~+)([^\r]*?[^~])\2(?!`)/gm, '<code>$3<\/code>');
                $("#markdown").html(str);
